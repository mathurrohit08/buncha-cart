...
export const FeaturedCollections = () => {
  ...
  return <div className="max-w-[1600px] mx-auto py-8 sm:py-12 px-4">
      <h2 className="text-2xl sm:text-3xl font-bold text-center mb-4 sm:mb-8">Featured Collections</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {featuredCollections.map(collection => <motion.div key={collection.name} whileHover={{
        scale: 1.02
      }} className="relative overflow-hidden rounded-lg shadow-md">
            <Link to={`/products/${collection.name.toLowerCase().replace(/\s+/g, "-")}`}>
              <div className="relative">
                <img src={collection.image} alt={collection.name} className="w-full h-40 sm:h-48 object-cover" onError={e => {
              (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1488590528505-98d2b5aba04b?auto=format&fit=crop&w=500&h=350";
                ...
                <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center text-center p-4">
                  ...
                  <Button size="sm" variant="outline" className="border-white text-zinc-950 bg-zinc-100">
                    Shop Now
                  </Button>
                </div>
              </div>
